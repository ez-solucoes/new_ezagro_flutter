...
import '../../../domain/entities/plot_entities/farm_plot_entity.dart';
...
import '../../widgets/custom_tiles/custom_two_items_alternate_color_selectable_label_widget.dart';
...
import '../../widgets/filter_bottom_sheet/filter_companies_bottom_sheet_widget.dart';
import '../../widgets/filter_bottom_sheet/filter_products_bottom_sheet_widget.dart';
import '../item_selection_controller.dart';

class EditListedItemsPage extends StatefulWidget {
  final ArgParams args;

  static const String routePath = AppRoutes.appEditListedItemsPage;

  static void navigate({required ArgParams args}) => Modular.to.navigate(routePath, arguments: args);
  ...
  @override
  State<EditListedItemsPage> createState() => _EditListedItemsPageState();
}

class _EditListedItemsPageState extends State<EditListedItemsPage> {
  final ItemSelectionController itemSelectionController = Modular.get<ItemSelectionController>();
  final serviceOrderCreateController = Modular.get<ServiceOrderCreateController>();
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      itemSelectionController.loadItems(
        currentItemType,
        filterParams: currentItemType == ItemType.plot
            ? ArgParams(
          thirdArgs: serviceOrderCreateController.agriculturalActivityType?.value,
          fourthArgs: serviceOrderCreateController.farm?.value,
          fifthArgs: serviceOrderCreateController.costCenterByCostCenterTypeId?.value,
          sixthArgs: serviceOrderCreateController.cropVariety?.value,
        )
            : null,
      );
  ...
  String _getEmptyListMessage(ItemType type) {
    switch (type) {
      ...
      case ItemType.plot:
        return 'Nenhum talhão encontrado. Verifique os filtros ou a busca.';
    }
  }
              ...
              children: [
                ...
                CustomFloatingActionButtonWidget(
                  onPressed: () {
                    ...
                    } else if(currentItemType == ItemType.plot){
                      selectedItems = itemSelectionController.finalRequestedPlots;
                    } else {
                      selectedItems = [];
                    }
        ...
        child: Observer(
          builder: (context) {
            final bool isLoadingInitialData = itemSelectionController.isFirstLoading ||
                ...
                (currentItemType == ItemType.plot && itemSelectionController.isPlotLoading);
              ...
              children: [
                Autocomplete<String>(
                  ...
                  optionsBuilder: (TextEditingValue textEditingValue) {
                    ...
                    List<SelectEntity> sourceListForAutocomplete; // Agora sempre SelectEntity
                    ...
                    if (currentItemType == ItemType.product) {
                      ...
                      sourceListForAutocomplete = itemSelectionController.plotListByFilterToSelect;
                    } else {
                      return const Iterable<String>.empty();
                    }
                ...
                Expanded(
                  child: Container(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          (currentItemType == ItemType.product || currentItemType == ItemType.agriculturalInput)
                            ...
                            secondLabel: secondLabel,
                              ...
                              builder: (context) {
                                ...
                                if (currentItemType == ItemType.company) {
                                  allSelected = itemSelectionController.areAllCompaniesSelected;
                                  onChangedCallback = (newValue) {
                                    itemSelectionController.toggleAllCompaniesSelection(newValue);
                                  };
                                } else if (currentItemType == ItemType.plot) {
                                  allSelected = itemSelectionController.areAllPlotsSelected;
                                  onChangedCallback = (newValue) {
                                    itemSelectionController.toggleAllPlotsSelection(newValue);
                                  };
                                } else {
                                  allSelected = false;
                                  onChangedCallback = (newValue) {};
                                ...
                                return CustomTwoItemsAlternateColorSelectableLabelWidget(
                                  firstLabel: firstLabel,
                                  secondLabel: secondLabel,
                                  initialIsSelected: allSelected,
                                  onChanged: onChangedCallback, // Chamada ao controlador
                                );
                              }
                          ),
                          const SizedBox(height: 10),
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  if (currentItemType == ItemType.product) {
                                    ...
                                    if (item is SelectEntity) {
                                      ...
                                      return Theme(
                                        ...
                                        child: ExpansionTile(
                                          ...
                                          tilePadding: const EdgeInsets.all(0),
                                          trailing: const SizedBox(width: 0.0),
                                          // NEW: Wrap CustomTwoItemsAlternateColorSelectableTileWidget in Observer
                                          title: Observer(
                                            builder: (_) {
                                              return CustomTwoItemsAlternateColorSelectableTileWidget(
                                    ...
                                    if (item is FarmPlotEntity) {
                                      final plotItem = item;

                                      final double? areaValue = double.tryParse(plotItem.area ?? '');
                                      final String areaLabel = areaValue?.toStringAsFixed(2) ?? 'N/A';
                                      ...
                                      return Theme(
                                        ...
                                        child: ExpansionTile(
                                          ...
                                          title: Observer(
                                            builder: (_) {
                                              return CustomTwoItemsAlternateColorSelectableTileWidget(
                                                firstLabel: plotItem.number ?? 'Talhão Desconhecido',
                                                secondLabel: '$areaLabel ha',
                                                initialIsSelected: itemSelectionController.isPlotSelected(plotItem.id),
                                                onChanged: (isSelected) {
                                                  itemSelectionController.togglePlotSelection(plotItem.id, isSelected);
                                                },
                                              );
                                            },
                                          ),
                                      ...
                                      debugPrint('Tipo de item inesperado para ItemType.plot: ${item.runtimeType}');
                                      return Container();
                                    }
                                  }
                                  return Container();
